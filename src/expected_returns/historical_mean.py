...
class HistoricalMeanReturn(ExpectedReturnModel):
    """
    과거 평균 수익률 기반 기대수익률 모델.
    ...
    def fit_predict(self, returns: pd.DataFrame) -> pd.Series:
        ...
        mu = filtered_returns.mean() * self.periods_per_year
        mu.name = "expected_return"

        return mu.sort_index()
